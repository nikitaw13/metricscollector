...
func NewCollectorMonitor(interval time.Duration) *CollectorMonitor {
	return &CollectorMonitor{
		Interval: interval,
	}
}

func (m *CollectorMonitor) Run(c *Collector) {
	for {
		...
		c.Storage.UpdateCounter("PollCount", 1) // TODO()
		c.Storage.UpdateGauge("RandomValue", rand.NormFloat64())
